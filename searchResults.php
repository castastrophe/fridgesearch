...
<div id="main">
	...
	<?php
		if(!$ingredients){
			echo "<p class=\"search\">No search terms were entered.</p>";
		}
		else{
			$searchterms="<p class=\"search\">Recipes which include ";
			if(sizeof($userPick)>1){
				for($a=0; $a<(sizeof($userPick)-1); $a++){
					$searchterms .= "<span class=\"searchcolor\">".$userPick[$a]."</span>, ";
				}
				
				$searchterms .= "and/or <span class=\"searchcolor\">".$userPick[$a]."</span>.";
			}
			else{
				$searchterms .= "<span class=\"searchcolor\">".$userPick[0]."</span>:";
			}
	...
	<?php
		...
		if($ingredients){
			...
			if($bestMatch){
				...
				echo "<p class=\"entry\">There are no recipes entered in the database at this time. <a href=\"register.php\">Register</a> today and add your own!</p>";
			}
		}
	?>
		</table>
